...
class PositionManager {
  ...
  handleRaddec(raddec, positions) {
    ...
    if(hasLocationFeatures) {
      spatem = compileLocationSpatem(raddec, current, collection);
    }
    else {
      spatem = compilePositionSpatem(raddec, current);
    }
 ...
 */
function compilePositionSpatem(raddec, position) {
  return {
 ...
 */
function compileLocationSpatem(raddec, position, collection) {
  let is3DPosition = (position.length === 3);
